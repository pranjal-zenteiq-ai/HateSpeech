...
def main():
    ...
    # Use large stratified sample (100k from ~726k) for practical evaluation
    print("Sampling from full dataset (stratified)...")
    df_test = (
        df.groupby("Label", group_keys=False)
        .apply(lambda x: x.sample(n=min(50000, len(x)), random_state=42))
        .reset_index(drop=True)
    )
    
    print(f"\nTest set size: {len(df_test)} (PRACTICAL EVALUATION SAMPLE FROM FULL 726K DATASET)")
    ...
    bc = BinaryClassifier(
        ...
        xgb_model_path="models/xgb_semantic_full.joblib",
        semantic_weight=0.7,
        lexical_weight=0.3
    )
    
    # Get predictions
    print("Generating predictions (using TF-IDF LR only for speed)...")
    texts = df_test["Content"].astype(str).tolist()
    y_true = df_test["Label"].values
    
    # Use only LR for speed, with XGB weighted 0
    print("  Computing LR probabilities...")
    lr_probs = bc._lr_probability(texts)
    
    # For practical speed, use only LR probabilities
    y_proba = lr_probs
    ...
    # ===========================
    print("\n" + "="*70)
    print("ENSEMBLE CLASSIFIER EVALUATION (100k samples from full 726k dataset)")
    print("="*70)
    ...
    print("\n[6] SAMPLE PREDICTIONS (First 15 Examples)")
    print("-" * 70)
    ...
    if len(false_positives) > 0:
        print("\n  Sample False Positives (Highest probability):")
        fp_indices = false_positives[np.argsort(y_proba[false_positives])[-5:]]
        for idx in fp_indices[::-1]:
            print(f"    Prob={y_proba[idx]:.4f}: {texts[idx][:100]}...")
    
    if len(false_negatives) > 0:
        print("\n  Sample False Negatives (Lowest probability):")
        fn_indices = false_negatives[np.argsort(y_proba[false_negatives])[:5]]
    ...
    print("(Note: Using LR predictions only for speed in this evaluation)")
